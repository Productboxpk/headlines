...
export async function getAllProjectIssues(accessToken, jiraBsaeURL, projectKey) {
    ...
    const { data } = await axios({
        method: "GET",
        url: `${jiraBsaeURL}/rest/api/3/search?expand=changelog&fields=all,summary,description,assignee,updated,status,updatedHistroy=true&jql=project in (${projectKey})`,
        headers: {
            Authorization: `Bearer ${accessToken}`
        }
    }).catch(err => console.log(err, "Orgs error"));
