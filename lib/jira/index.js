import { findByClientKey, findByUserAccountId } from "../models/installation";
...
export const token = async (clientKey, userAccountId = null) => {
    try {
        let data = null;
        if (clientKey) {
            data = await findByClientKey(Installations, clientKey);
        } else {
        ...
        }, { where: { client_key: clientKey || data.client_key } })
        const updatedClient = await findByClientKey(Installations, clientKey || data.client_key)
