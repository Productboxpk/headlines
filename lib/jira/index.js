...
export const token = async (req, res, next) => {
    try {
        ...
        const accessTokenData = await getAccessToken(options).catch(err => console.log(err, "Token err"));
        console.log(accessTokenData,'access token data');
        if (accessTokenData) {
            const updatedClient = await Installations.update({
 ...
 function getAccessToken(opts) {
    console.log('get Access token',opts,'***********');
    opts = opts || {};
